...
//to display the details of a selected item
 async function selectedItem (k){
        ...
        data1.data.recipe.ingredients.forEach(element => {
            array1.push({quantity:`${element.quantity}`,unit:`${element.unit}`,description:`${element.description}`}) 
        });    
        ingredients();
}


//to print ingredients in the web page
function ingredients(){
    console.log(array1);
    let temp1;
        array1.forEach(element=>{
            console.log(typeof(parseFloat(`${element.quantity}`)));
            if(isNaN(parseInt(`${element.quantity}`))){
                temp1 ="";
            }
            else{
                temp1=`${element.quantity}`
            }
        ingredients_list.innerHTML +=`<li>${temp1}  ${element.unit}  ${element.description}</li>`
     }) 
}
...
//to increase the no of servings
plus.addEventListener("click",()=>{
    serving++;
    console.log(serving);
    no_of_servings.innerHTML=serving;
    ingredients_list.innerHTML ="";
    for(i=0;i<data1.data.recipe.ingredients.length;i++){
        array1[i].quantity=((parseFloat(`${data1.data.recipe.ingredients[i].quantity}`)/parseFloat(`${data1.data.recipe.servings}`))*serving).toFixed(2);
         console.log(array1);
    }
    ingredients()
})
...
//to decrease the no of servings

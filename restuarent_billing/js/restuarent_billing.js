...
// to add selected items into the billing
 function billing(elementid){
    var index = cart.map(function(o) { return o.Name; }).indexOf( data[elementid-1].itemName);
    console.log(index)
    if(index===-1){
        ...
        let temp= (cart[index].cost)/cart[index].quantity;
            cart[index].quantity= cart[index].quantity+1;
            cart[index].cost=parseInt(cart[index].cost)+temp;
...
function totalcount(){
   ...
   taxamount=parseFloat(totalprice*0.04.toFixed(2));
   paybleamount=(totalprice+taxamount).toFixed(2);
